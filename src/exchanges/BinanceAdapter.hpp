...
public:
    BinanceAdapter(const std::string& config_path);
    ~BinanceAdapter() override;

    // Prevent copying
    BinanceAdapter(const BinanceAdapter&) = delete;
    BinanceAdapter& operator=(const BinanceAdapter&) = delete;
...
private:
    ...
    // WebSocket streams
    std::unordered_map<std::string, std::string> ws_streams_;
